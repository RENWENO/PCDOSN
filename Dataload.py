...
#获取属性字典和特征字典,以及属性种类
with open("../data/Facebook/107.featnames","r",encoding="utf-8") as f4:
    featdict = {}
    attributedict = {}
...
#获取所有用户特征
with open("../data/Facebook/107.feat",'r',encoding="utf-8") as f5:
...
with open("../data/Facebook/107.edges","r") as f4:
    edgs = f4.readlines()
    friendlist = []
    friendstrlist =[]
    alllablelist=[]
...
for i,key in  enumerate(keylist1):
    ...
    if len(friendstrlist[i]) != arrays_loaded1[key].shape[0]-576:
        print(key, "朋友个数：", len(friendstrlist[i]))
        print(key, "朋友个数：", arrays_loaded1[key].shape[0] - 576)
