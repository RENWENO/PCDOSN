...
from HAN.LNFfusion import LayerNormDotProductAttentionFusion
...
class HAN(nn.Module):
    def __init__(
        ...
        self.yce = nn.Linear(350,45,bias=True)
    ...
    def forward(self, g, h):
        ...
        #newfture = self.fusion(h2,h)
        T = self.pool(g,h)
        #T = F.elu(T)
        T = self.liner4(T)
        newT = self.fusion(h2,h,g)
        T = self.liner1(newT+T)
        T = self.liner2(T)
        #T = self.bn2(T)
        T = F.elu(T)
        yc = self.yce(T)
